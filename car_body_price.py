import pandas as pd
from const import CAR_MAKE_BODY_MODELS

car_cost_class = pd.read_excel('car_classes.xlsx')

body_class = pd.DataFrame(columns=['make', 'model', 'class'])

for make in CAR_MAKE_BODY_MODELS:
    for body in CAR_MAKE_BODY_MODELS[make]:
        for model in CAR_MAKE_BODY_MODELS[make][body]:
